...
namespace WebGetApi.Controllers
{
    ...
    public class gwd_SecurityBureau_itemsController : ApiController
    {
        ...
        public async Task<IHttpActionResult> Post(entityCommMain entityCommMain)
        {
            ...
            try
            {
                ...
                if (entityCommMain.gwd_SecurityBureau_items != null)
                {
                    foreach (var item in entityCommMain.gwd_SecurityBureau_items)
                    {
                    ...
                    if (entityCommMain.gwd_SecurityBureau_items.Count > 0)
                    {
                        var tmpfirst = entityCommMain.gwd_SecurityBureau_items.First();

                        var tmpExitItem = gwd_SecurityBureau_itemsExists(tmpfirst.tkeyNo, tmpfirst.tLang, tmpfirst.tIndex);
                        if (tmpExitItem)
                        {
                            long tmpHtmlId = 0;
                            foreach (var item in entityCommMain.gwd_SecurityBureau_items)
                            {
                                ...
                                {
                                    var getItem = gwd_SecurityBureau_itemsByNo(item.tkeyNo, item.tLang, item.tIndex);
                                    if (getItem != null)
                                    {
                                        ...
                                        getItem.LicenceNo = item.LicenceNo;
                                    ...
                                    {
                                        tmpMain.gwd_SecurityBureau_items.Add(getItem);
                                    }
        ...
        private bool gwd_SecurityBureau_itemsExists(string id, long lang, long tindex)
        {
            return db.gwd_SecurityBureau_items.Count(e => e.tkeyNo == id && e.tLang == lang) > 0;
        }
        private gwd_SecurityBureau_items gwd_SecurityBureau_itemsByNo(string id, long lang, long tindex)
        {
            return db.gwd_SecurityBureau_items.Where(e => e.tkeyNo == id && e.tLang == lang).FirstOrDefault();
        }
