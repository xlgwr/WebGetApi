...
namespace WebGetApi.Controllers
{
    ...
    /// </summary>
    [RoutePrefix("api/GWDSecretaries")]
    public class gwd_Secretaries_itemsController : ApiController
    {
        ...
        public async Task<IHttpActionResult> Post(entityCommMain entityCommMain)
        {
            ...
            try
            {
                ...
                if (entityCommMain.gwd_Secretaries_items != null)
                {
                    foreach (var item in entityCommMain.gwd_Secretaries_items)
                    {
                    ...
                    if (entityCommMain.gwd_Secretaries_items.Count > 0)
                    {
                        var tmpfirst = entityCommMain.gwd_Secretaries_items.First();

                        var tmpExitItem = gwd_Secretaries_itemsExists(tmpfirst.tkeyNo, tmpfirst.tLang, tmpfirst.tIndex);
                        if (tmpExitItem)
                        {
                            long tmpHtmlId = 0;
                            foreach (var item in entityCommMain.gwd_Secretaries_items)
                            {
                                ...
                                {
                                    var getItem = gwd_Secretaries_itemsByNo(item.tkeyNo, item.tLang, item.tIndex);
                                    if (getItem != null)
                                    {
                                    ...
                                    {
                                        tmpMain.gwd_Secretaries_items.Add(getItem);
                                    }
        ...
        private bool gwd_Secretaries_itemsExists(string id, long lang, long tindex)
        {
            return db.gwd_Secretaries_items.Count(e => e.tkeyNo == id && e.tLang == lang) > 0;
        }
        private gwd_Secretaries_items gwd_Secretaries_itemsByNo(string id, long lang, long tindex)
        {
            return db.gwd_Secretaries_items.Where(e => e.tkeyNo == id && e.tLang == lang).FirstOrDefault();
        }
