...
namespace WebGetApi.Controllers
{
    ...
    /// </summary>
    [RoutePrefix("api/GWDarchitect")]
    public class gwd_architect_itemsController : ApiController
    {
        ...
        public async Task<IHttpActionResult> Post(entityCommMain entityCommMain)
        {
            ...
            try
            {
                ...
                if (entityCommMain.gwd_architect_items != null)
                {
                    foreach (var item in entityCommMain.gwd_architect_items)
                    {
                    ...
                    if (entityCommMain.gwd_architect_items.Count > 0)
                    {
                        var tmpfirst = entityCommMain.gwd_architect_items.First();
                        ...
                        if (tmpExitItem)
                        {
                            ...
                            foreach (var item in entityCommMain.gwd_architect_items)
                            {
                                if (!string.IsNullOrEmpty(item.MembershipNo))
                                {
                                    ...
                                    if (getItem != null)
                                    {
                                        ...
                                        getItem.MembershipNo = item.MembershipNo;
                                        getItem.MembershipNoZH = item.MembershipNoZH;
                                    ...
                                    {
                                        tmpMain.gwd_architect_items.Add(getItem);
                                    }
        ...
        {
            return db.gwd_architect_items.Count(e => e.tkeyNo == id && e.tLang == lang) > 0;
        }
        private gwd_architect_items gwd_architect_itemsByNo(string id, long lang, long tindex)
        {
            return db.gwd_architect_items.Where(e => e.tkeyNo == id && e.tLang == lang).FirstOrDefault();
        }
