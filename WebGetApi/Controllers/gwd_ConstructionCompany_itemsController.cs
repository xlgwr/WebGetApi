...
namespace WebGetApi.Controllers
{
    ...
    public class gwd_ConstructionCompany_itemsController : ApiController
    {
        ...
        public async Task<IHttpActionResult> Post(entityCommMain entityCommMain)
        {
            ...
            try
            {
                ...
                if (entityCommMain.gwd_ConstructionCompany_items != null)
                {
                    foreach (var item in entityCommMain.gwd_ConstructionCompany_items)
                    {
                    ...
                    if (entityCommMain.gwd_ConstructionCompany_items.Count > 0)
                    {
                        var tmpfirst = entityCommMain.gwd_ConstructionCompany_items.First();

                        var tmpExitItem = gwd_ConstructionCompany_itemsExists(tmpfirst.tkeyNo, tmpfirst.tLang, tmpfirst.tIndex);
                        if (tmpExitItem)
                        {
                            long tmpHtmlId = 0;
                            foreach (var item in entityCommMain.gwd_ConstructionCompany_items)
                            {
                                ...
                                {
                                    var getItem = gwd_ConstructionCompany_itemsByNo(item.tkeyNo, item.tLang, item.tIndex);
                                    if (getItem != null)
                                    {
                                    ...
                                    {
                                        tmpMain.gwd_ConstructionCompany_items.Add(getItem);
                                    }
        ...
        private bool gwd_ConstructionCompany_itemsExists(string id, long lang, long tindex)
        {
            return db.gwd_ConstructionCompany_items.Count(e => e.tkeyNo == id && e.tLang == lang) > 0;
        }
        private gwd_ConstructionCompany_items gwd_ConstructionCompany_itemsByNo(string id, long lang, long tindex)
        {
            return db.gwd_ConstructionCompany_items.Where(e => e.tkeyNo == id && e.tLang == lang).FirstOrDefault();
        }
