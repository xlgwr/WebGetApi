    ...
    public class i_BuildingComController : ApiController
    {
        ...
        public async Task<IHttpActionResult> Post(entityCommMain entityCommMain)
        {
            ...
            try
            {
                ...
                if (entityCommMain.i_BuildingCom != null)
                {
                    foreach (var item in entityCommMain.i_BuildingCom)
                    {
                    ...
                    if (entityCommMain.i_BuildingCom.Count > 0)
                    {
                        var tmpfirst = entityCommMain.i_BuildingCom.First();

                        var tmpExitItem = i_BuildingComCompany_itemsExists(tmpfirst.tkeyNo, tmpfirst.tLang, tmpfirst.tIndex);
                        if (tmpExitItem)
                        {
                            long tmpHtmlId = 0;
                            foreach (var item in entityCommMain.i_BuildingCom)
                            {
                                ...
                                {
                                    var getItem = i_BuildingComCompany_itemsByNo(item.tkeyNo, item.tLang, item.tIndex);
                                    if (getItem != null)
                                    {
                                        ...
                                        getItem.AddressEn = item.AddressEn;
                                        getItem.AddressCn = item.AddressCn;
                                    ...
                                    {
                                        tmpMain.i_BuildingCom.Add(getItem);
                                    }
        ...
        private bool i_BuildingComCompany_itemsExists(string id, long lang, long tindex)
        {
            return db.i_BuildingCom.Count(e => e.tkeyNo == id && e.tLang == lang) > 0;
        }
        private i_BuildingCom i_BuildingComCompany_itemsByNo(string id, long lang, long tindex)
        {
            return db.i_BuildingCom.Where(e => e.tkeyNo == id && e.tLang == lang).FirstOrDefault();
        }
