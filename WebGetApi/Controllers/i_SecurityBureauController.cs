...
namespace WebGetApi.Controllers
{
    ...
    /// </summary>
    [RoutePrefix("api/GWDSecurityBureau")]
    public class i_SecurityBureauController : ApiController
    {
        ...
        public async Task<IHttpActionResult> Post(entityCommMain entityCommMain)
        {
            ...
            try
            {
                ...
                if (entityCommMain.i_SecurityBureau != null)
                {
                    foreach (var item in entityCommMain.i_SecurityBureau)
                    {
                    ...
                    if (entityCommMain.i_SecurityBureau.Count > 0)
                    {
                        var tmpfirst = entityCommMain.i_SecurityBureau.First();

                        var tmpExitItem = i_SecurityBureauExists(tmpfirst.tkeyNo, tmpfirst.tLang, tmpfirst.tIndex);
                        if (tmpExitItem)
                        {
                            long tmpHtmlId = 0;
                            foreach (var item in entityCommMain.i_SecurityBureau)
                            {
                                if (!string.IsNullOrEmpty(item.CompanyNameEn))
                                {
                                    var getItem = i_SecurityBureauByNo(item.tkeyNo, item.tLang, item.tIndex);
                                    if (getItem != null)
                                    {
                                        tmpHtmlId = getItem.htmlID;

                                        getItem.tIndex = item.tIndex;
                                        getItem.CompanyNameEn = item.CompanyNameEn;
                                        getItem.CompanyNameCn = item.CompanyNameCn;
                                        getItem.SecurityComId = item.SecurityComId;

                                        getItem.address = item.address;
                                        getItem.WorkType = item.WorkType;
                                    ...
                                    {
                                        tmpMain.i_SecurityBureau.Add(getItem);
                                    }
                            ...
                            //add is not exit
                            if (tmpMain.i_SecurityBureau.Count > 0)
                            {
        ...
        private bool i_SecurityBureauExists(string id, long lang, long tindex)
        {
            return db.i_SecurityBureau.Count(e => e.tkeyNo == id && e.tLang == lang) > 0;
        }
        private i_SecurityBureau i_SecurityBureauByNo(string id, long lang, long tindex)
        {
            return db.i_SecurityBureau.Where(e => e.tkeyNo == id && e.tLang == lang).FirstOrDefault();
        }
